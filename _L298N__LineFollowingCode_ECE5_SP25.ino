...
const int I_pin = 2; // Pin connected to I term potentiometer
const int D_pin = 1; // Pin connected to D term potentiometer //LEFT MOST potentiometer
                                                                 
int led_Pins[] = {46};  // RED LED set to pin 46 (by the BIG Ant and Pete)

//BIG tips by the BIG K: FOR more secure MOTOR attachment
// - screws can be screwed in from outside the hole of the chassis so they are secure 
// (right now they are screwed in the motor already before even attaching to the chassis
...
void RunMotors() {
  M1SpeedtoMotor = min(NOMINALSPEED + SpRead + M1P, 255) * -1; // limits speed to 255
